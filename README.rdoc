...
Handle the setting of search_path in your ApplicationController like so (when tenant is your application user model):
  ...
  def handle_search_path
    if current_user
      PgTools.set_search_path current_user.tenant_schema_name
    else
      PgTools.restore_default_search_path
    end 
  end

If you want to set your tenant based on a subdomain, use the following in ApplicationController:
  before_filter :handle_search_path

  def handle_search_path
    @tenant = MyTenantAwareModel.find_by_subdomain(request.subdomain)
    if @tenant
